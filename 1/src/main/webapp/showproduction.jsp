        ...
        <div class="row">
          <%
            ...
            while (iterator.hasNext()) {
              ...
              //Грузим альбомы
              //Из всех альбомов выбираем только те что в этом альбоме
              if(pdGetAll.getId() == Integer.parseInt(request.getParameter("productionId"))){
                 pdGetAll.getAlbumName();
              }
              if(pdGetAll.getProductionByLink() != null){
              if(pdGetAll.getProductionByLink().equals(request.getAttribute("pdRead"))){
          %>
          <div class="col-sm-4 thumb-2">
            <h2><%= pdGetAll.getAlbumName()%></h2>
            <a href="MainServlet?action=showProductionAlbom&productionAlbomId=<%= pdGetAll.getId()%>&rootAlbomId=<%= ((ProductionEntity)(request.getAttribute("pdRead"))).getId()%>">
